...
import ActiveUser from "../ui-components/ActiveUser";

const Dashboard = ({ user, users }) => {
  return (
    <div className="animate-slide-down flex flex-col py-8 pl-6 pr-2 w-64 bg-white flex-shrink-0">
      ...
      <div className="flex flex-col mt-8">
        <div className="flex flex-row items-center justify-between text-xs">
          ...
          <span className="flex items-center justify-center bg-gray-300 h-4 w-4 rounded-full p-3 text-gray-800 font-semibold">
            {Object.keys(users).length}
          </span>
        </div>
        <div className="flex flex-col space-y-1 mt-4 -mx-2 h-48 overflow-y-auto">
          {/* {users.length > 0 && users.map((user) => <ActiveUser user={user} />)} */}
          {Object.keys(users).length > 0 ? (
            Object.keys(users).map((key) => (
              <ActiveUser key={key} user={users[key]} />
            ))
          ) : (
            <p>No active users.</p>
          )}
        ...
        {/* <div className="flex flex-row items-center justify-between text-xs mt-6">
          <span className="font-bold">Recently left</span>
